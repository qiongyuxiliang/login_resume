...
class TaskListRepository {
    ...
    login(req, res) {
        ...
        normalCon(user(req.body.account, req.body.password), function (err, row, fields) {
            console.log(row)
            if (row[0]['count(0)'] > 0) {
                res.json({
                    url:'/public/resume/index.html'
                })
                // res.sendFile(path.resolve(__dirname + '/../../public/resume/index.html'));
            }else{
                res.json({
                    url:''
                })
